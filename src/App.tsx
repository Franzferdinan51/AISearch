...
function App() {
  ...
  const [curationMode, setCurationMode] = useState<'ai' | 'partial' | 'heuristic' | 'disabled' | 'none'>('none')
  ...
  const loadResultsPage = async (page: number) => {
    ...
    try {
      ...
      if (diagnostics.length) setApiError(`AI diagnostics · ${diagnostics[0].provider || provider.name} ${diagnostics[0].stage || 'request'}: ${diagnostics[0].message}`)
      else if (!payload.results?.length && payload.errors?.length) setApiError(`No results were retrieved: ${payload.errors[0].message}`)
      else if (!payload.results?.length) setApiError('No results were retrieved. Try a more specific query or another search category.')
  ...
  const updateProvider = (id: string, field: 'endpoint' | 'model' | 'apiKey', value: string) => {
    if (field === 'endpoint' || field === 'model') activeRequest.current?.abort()
    setProviders((current) => current.map((item) => item.id === id ? { ...item, [field]: value } : item))
  }

  const selectProvider = (id: string) => {
    if (id !== selectedProvider) activeRequest.current?.abort()
  ...
  const cancelActiveSearch = () => { activeRequest.current?.abort(); activeRequest.current = null; setRunning(false) }
  const openHistory = (session: SearchSession) => { cancelActiveSearch(); setQuery(session.query); setSourceList(session.sources); setAnswer(session.answer); setResultsPage(1); setHasMoreResults(true); setView('search') }
  const newSearch = () => { cancelActiveSearch(); const nextMode = preferences.defaultMode; setView(nextMode === 'Deep research' || nextMode === 'Explore' ? 'research' : 'search'); setMode(nextMode); setQuery(''); setInput(''); setSourceList([]); setAnswer(''); setTraceSteps([]); setApiError(''); setRuntimeStages([]); setResultsPage(1); setHasMoreResults(true); setCurationMode('none') }
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <div className="select-wrap provider-select">
              ...
              {showProvider && <Dropdown custom items={providers.map((item) => `${item.name} • ${item.model}`)} onSelect={(item) => { const found = providers.find((p) => `${p.name} • ${p.model}` === item); if (found) selectProvider(found.id); else setShowProvider(false) }} />}
            </div>
...
function EmptySearch({ input, mode, provider, models, currentMissing, onModelChange, onInput, onModeChange, onSearch }: { input: string; mode: Mode; provider: Provider; models: ModelOption[]; currentMissing: boolean; onModelChange: (value: string) => void; onInput: (value: string) => void; onModeChange: (mode: Mode) => void; onSearch: (query: string) => void }) {
  const suggestions = [
