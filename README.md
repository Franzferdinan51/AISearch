...
Remote API keys can be supplied either through server-side `.env` variables or in the Provider screen for the current browser session. Browser-entered keys are sent only to the local Lumen API for the request being made and are stripped before provider settings are saved to localStorage. Provider settings provide model dropdowns for every built-in provider and let you choose the default used for new searches. Saved models and defaults are stored locally in the browser.

LM Studio uses its local OpenAI-compatible endpoint. Lumen accepts either a base server URL such as `http://host:1234` or an explicit `/v1` URL, discovers installed models from the local model API, and includes a masked **Server API key** field for servers with authentication enabled. Select a model, then use **Test model connection**; a `Connected · … answered` message proves Lumen sent a real completion to it. The token is sent only to the local Lumen API for model discovery and model requests, is held for the browser session, and is never written to localStorage. `LM_API_TOKEN` remains available as a server-side default. Local completions get a three-minute request budget by default, rather than the cloud-model budget; set `LM_STUDIO_TIMEOUT_MS` (30,000–600,000 ms) for slower hardware or larger models. Without a token, Lumen keeps working with other providers and displays an actionable local-only message instead of failing the page.

### Local-model reliability

Lumen profiles each LM Studio endpoint/model after **Test model connection** or **Warm & probe**. The profile keeps only local runtime metadata—format compatibility, warm-up state, timings, queue state, stage success history, and user-selected overrides—never API keys or raw prompt text. The Provider card exposes automatic format detection plus per-model timeout and retry controls; the API also provides `GET`, `PUT`, and `DELETE /api/local-runtime/profile` and `POST /api/local-runtime/probe` for local tooling. Local completions are queued, retried once by default with a smaller budget for retryable failures, and cached briefly by stage. Each search-result page is retrieved and ranked independently, so later pages keep their natural lower-rank order and do not trigger a multi-page SearXNG burst. Search results still return if any local AI stage is slow, unavailable, or malformed.
